...
# Find the average rating(the average of ratings user gave to each category of movie) for each user and stores it in the user's object
...
# Perform clustering on MOVIE ITEMS
# print item[0].title
movie_genre = []
for movie in item:
...
# print movie_genre
ask = random.sample(item, 1000)
new_user = np.zeros(19)

print "Please rate the following movies (1-5):"
cnt=0
for movie in ask:                       # assign new clusters to every user
	print movie.title + ": "
	str = raw_input("have you watched this movie? yes OR no :: ");
	if str=="no": 
		continue
	a = int(input())
...
#new utility matrix will contain (old utility matrix + new row of new_user ratings)
...
top_5 = [x for (y,x) in sorted(zip(pcs_matrix, user_index), key=lambda pair: pair[0], reverse=True)]    #find top 5 user closest to new user entered
top_5 = top_5[:5]
# print top_5
print
top_5_genre = []

for i in range(0, 5):     #find category to which top 5 users belong to
	maxi = 0
	maxe = 0
	for j in range(0, 19):
...
print "Movie genres and Movies you'd like:"
print 
for i in top_5_genre:
	...
	for movie in item:
		if(i==0 and movie.unknown==1):
			print movie.title
			break
		elif(i==1 and movie.action==1):
			print movie.title
			break
		elif(i==2 and movie.adventure==1):
			print movie.title
			break
		elif(i==3 and movie.animation==1):
			print movie.title
			break
		elif(i==4 and movie.childrens==1):
			print movie.title
			break
		elif(i==5 and movie.comedy==1):
			print movie.title
			break
		elif(i==6 and movie.crime==1):
			print movie.title
			break
		elif(i==7 and movie.documentary==1):
			print movie.title
			break
		elif(i==8 and movie.drama==1):
			print movie.title
			break
		elif(i==9 and movie.fantasy==1):
			print movie.title
			break
		elif(i==10 and movie.film_noir==1):
			print movie.title
			break
		elif(i==11 and movie.horror==1):
			print movie.title
			break
		elif(i==12 and movie.musical==1):
			print movie.title
			break
		elif(i==13 and movie.mystery==1):
			print movie.title
			break
		elif(i==14 and movie.romance==1):
			print movie.title
			break
		elif(i==15 and movie.sci_fi==1):
			print movie.title
			break
		elif(i==16 and movie.thriller==1):
			print movie.title
			break
		elif(i==17 and movie.war==1):
			print movie.title
			break
		elif(i==18 and movie.western==1):
			print movie.title
			break
